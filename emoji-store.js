...
export const createEmojiStore = origin => {
  ...
  const initEmojis = async () => {
    ...
    const moreThan24HoursHavePassed =
      Date.now() - Number(localStorage.getItem(`tlEmojis${host}Loaded`)) >= 24 * 60 * 60 * 1000;

    if (
      localStorage.getItem(`tlEmojis${host}`) &&
      localStorage.getItem(`tlEmojis${host}Loaded`) &&
      !moreThan24HoursHavePassed
    ) {
  ...
  const emojiShortcodeToUrl = async (name, host) => {
    ...
    if (
      !(resolvedHost in emojiShortcodeToUrlDic) ||
      !(name in emojiShortcodeToUrlDic[resolvedHost])
    ) {
